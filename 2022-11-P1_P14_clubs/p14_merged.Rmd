...
p14_merged_scale_pca <- RunPCA(object = p14_merged_subset_vf_scale,
                               features = VariableFeatures(object = p14_merged_subset_vf_scale))
...
##neuronal markers
#synaptobrevin
p14_merged_nsyb <- GetAssayData(object = p14_merged_normalize,
...
p14_merged_nsyb_subset <- subset(x = p14_merged_nsyb_df, subset = nsyb_count > 2) ##1334

##embryonic lethal abnormal vision
p14_merged_elav <- GetAssayData(object = p14_merged_normalize,
...
p14_merged_elav_subset <- subset(x = p14_merged_elav_df, subset = elav_count > 2) ##1304

##brunchpilot
p14_merged_brp <- GetAssayData(object = p14_merged_normalize,
...
p14_merged_brp_subset <- subset(x = p14_merged_brp_df, subset = brp_count > 2) ##23903

##cadherin N
p14_merged_cadn <- GetAssayData(object = p14_merged_normalize,
...
p14_merged_cadn_subset <- subset(x = p14_merged_cadn_df, subset = cadn_count > 2) ##18404

##synaptotagmin
p14_merged_syt1 <- GetAssayData(object = p14_merged_normalize,
...
p14_merged_syt1_subset <- subset(x = p14_merged_syt1_df, subset = syt1_count > 2) ##15195

##glial marker
#reverse polarity
p14_merged_repo <- GetAssayData(object = p14_merged_normalize,
...
p14_merged_repo_subset <- subset(x = p14_merged_repo_df, subset = repo_count > 2) ##487
...
colnames(p14_merged_neuronal_uniq_ids) <- "p14_merged_neuronal_uniq_ids" ##21672 nuclei

##subset the normalized seurat object for the barcodes with at least two neuronal markers
p14_merged_normalize_neuron_ids <- subset(x = p14_merged_normalize,
                                  cells = p14_merged_neuronal_uniq_ids$p14_merged_neuronal_uniq_ids)
length(colnames(p14_merged_normalize_neuron_ids))##21672 nuclei
length(rownames(p14_merged_normalize_neuron_ids)) ##14238 genes
...
p14_merged_normalize_neuron_ids <- RunPCA(object = p14_merged_normalize_neuron_ids,
                             features = VariableFeatures(object = p14_merged_normalize_neuron_ids))
DimHeatmap(p14_merged_normalize_neuron_ids, dims = 1:15, cells = 500, balanced = TRUE)

##cluster the nuclei
p14_merged_normalize_neuron_ids <- FindNeighbors(object = p14_merged_normalize_neuron_ids,
                                         dims = 1:10)
p14_merged_normalize_neuron_ids <- FindClusters(object = p14_merged_normalize_neuron_ids,
                                        resolution = 0.5)
p14_merged_normalize_neuron_ids <- RunTSNE(object = p14_merged_normalize_neuron_ids,
                                   dims = 1:10)
...
gene_exp_per_cell <- data.frame(p14_merged_normalize_neuron_ids@assays[["RNA"]]@data)
p14_merged_gene_id <- rownames(gene_exp_per_cell)
gene_exp_per_cell$gene_id <- p14_merged_gene_id
rownames(gene_exp_per_cell) <- NULL
gene_exp_per_cell$gene_id <- as.factor(gene_exp_per_cell$gene_id)
p14_merged_or_per_cell <- gene_exp_per_cell[gene_exp_per_cell$gene_id %in% or_genes$or_genes, ]
...
length(p14_merged_or_per_cell_t$ObirOr5.9E1) ##this should be 21672 (# of neuronal cells)
...
length(p14_merged_cell_barcode_vec)  #566 nuclei
...
for (position in seq_len(length(p14_merged_cell_barcode_vec))) {
  ...
  p14_merged_table <- data.frame(table(p14_merged_or_per_cell_t[cell_barcode, ] > 2 )) 
...
p14_merged_only_one_or <- subset(x = p14_merged_result_df, subset = true_or == 1) ##26

##subset the nuclei for which there are more than 1 ORs with read count > 1
p14_merged_more_than_one_or <- subset(x = p14_merged_result_df, subset = true_or > 1) ##540
...
p14_merged_more_than_four_ors <- subset(x = p14_merged_result_df, subset = true_or >= 4) ##397

#subset for nuclei with no ORs with read count > 1
p14_merged_no_or <- subset(x = p14_merged_result_df, subset = true_or == 0) ##0

#make a freq distribution
p14_merged_categories <- c("no_OR", "only_one_OR",
                "two_or_less_than_four", "four_or_more_ORs")
p14_merged_values <- c(0, 26, 143, 397)
